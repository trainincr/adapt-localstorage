define(['coreJS/adapt'], function(Adapt) {
    ...
    var river_localstorage = _.extend({
       ...
       initialize: function() {       	
		...
		onConfigLoaded: function(){
			this._config = Adapt.config.has('_localStorage') ? Adapt.config.get('_localStorage') : false;
			
			if(this._config && this._config._isEnabled !== false){
				console.log("_localStorage inicializado");
	...
	var registerView = ModalView.extend({
    	templateHTMLInfo:   "<div>"+
                                "<div style='padding:20px'>"+
                                "<h3>Info de usuario</h3>"+
                                "<hr>"+
                                "<strong>Usuario registrado: </strong><label>"+localStorage.getItem("user_pj")+"</label><br><br>"+
                                "<strong>Estatus de avance: </strong><label>"+'Incompleto'+"</label><br><br>"+
                                "<a href='#'' id='btn_reset' class='button reset'>"+
                                    "<span>Reiniciar</span>"+
                                "</a>"+
                                "<hr>"+
                            "</div>",

        templateHTMLNewUser: "<div>"+
                                "<div style='padding:20px'>"+                             
                                "<hr>"+
                                "<label>Usuario: </label>"+
                                "<input id='inputuser' name='user' type='text'/><br><br>"+
                                "<a href='#'' id='btn_register' class='button submit'>"+
                                    "<span>Ingresar</span>"+
                                "</a>"+
                                "<p id='warning-user'></p>"+
                                "<hr>"+
    	...
    	initialize: function(){
            ...
            if(localStorage.getItem("user_pj")){
                this.template = _.template(this.templateHTMLInfo);
            }else{
                this.template = _.template(this.templateHTMLNewUser);
            }
        ...
        events:
        {
            "click #btn_reset": "resetUser",
            "click #btn_register": "register"
        },

        resetUser: function(){
            console.log('reset presionado');
            localStorage.setItem("user_pj", "");
        },

        register: function(){              
            if($('#inputuser').val().length === 0 ) {
                console.log('Textbox vacio');
                $('#warning-user').addClass('warning');
            }else{
                localStorage.setItem("user_pj", $('#inputuser').val());
                localStorage.setItem("estatus_pj", 'Sin iniciar');
                this.hideModal();
            }            
        },
